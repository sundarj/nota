...
exports.default = {
  ...
  created: function created() {
    ...
    _bus2.default.$on('focus', function (id) {
      return _this.$nextTick(function (_) {
        return _this.focus(document.getElementById(id));
      });
    });
  },
  ...
  methods: {
    ...
    keyup: function keyup(event) {
      ...
      if (keyPressed === 'escape') this.cancel(target);else if (keyPressed === 'enter') this.blur(target);
    }
  }
};
})()
...
var __vue__options__ = (typeof module.exports === "function"? module.exports.options: module.exports)
if (__vue__options__.functional) {console.error("[vueify] functional components are not supported and should be defined in plain js files using render functions.")}
__vue__options__.render = function(){with(this){return _h('nav',{staticAttrs:{"am-nota":"list"},on:{"!click":function($event){edit( $event )},"keyup":function($event){keyup( $event )}}},[(results)&&_l((results),function(item){return [(item.type === "nota")?_h('a',{directives:[{name:"href",value:({ name: "root", params: { id: item.id } }),expression:"{ name: \"root\", params: { id: item.id } }"}],staticAttrs:{"am-nota":"item"}},[_m(0)," ",_h('input',{directives:[{name:"model",value:(item.title),expression:"item.title"}],props:{"value":_s(item.title)},attrs:{"data-id":item.id,"id":item.id},staticAttrs:{"readonly":""},on:{"input":function($event){if($event.target.composing)return;item.title=$event.target.value}}})]):_h('a',{directives:[{name:"href",value:({ name: "item", params: { id: item.id } }),expression:"{ name: \"item\", params: { id: item.id } }"}],staticAttrs:{"am-nota":"folder-title"}},[_h('span',{staticClass:"material-icons"},[" folder "])," ",_h('input',{directives:[{name:"model",value:(item.title),expression:"item.title"}],props:{"value":_s(item.title)},attrs:{"data-id":item.id,"id":item.id},staticAttrs:{"readonly":""},on:{"input":function($event){if($event.target.composing)return;item.title=$event.target.value}}})])," "]})])}}
